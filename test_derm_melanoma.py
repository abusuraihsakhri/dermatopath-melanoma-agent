import pytest
from derm_melanoma import BreslowMicrostagingAgent, MitoticRateAuditorAgent, PeripheralMarginAgent, MelanomaCoordinator, main


def test_sub_agents():
    a1 = BreslowMicrostagingAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = MitoticRateAuditorAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = PeripheralMarginAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = MelanomaCoordinator()
...
def test_domain_registry():
    from derm_melanoma import DomainKnowledgeRegistry
