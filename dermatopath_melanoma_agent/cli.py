"""
Command-Line Interface for DermatoPath Sentinel: Cutaneous Melanoma Microstaging & Margin Safety Agent.
"""
import argparse
import csv
import sys
from .models import ClinicalCasePayload
from .agents import MelanomaCoordinator
...
def main(argv=None):
    ...
    if args.command == "audit":
        ...
        print(f"  Domain: Dermatopathology | Standard: AJCC 8th Edition & CAP Melanoma Protocol")
    ...
    if args.command == "batch":
        ...
        for idx, r in enumerate(rows):
            try:
                case = ClinicalCasePayload(
                    ...
                    is_stat=r.get("is_stat", r.get("critical_flag", "")).lower() in ("true", "1", "yes"),
                )
                dossier = coordinator.process_case(case)
                row_dict = dict(r)
                ...
                errors.append(f"Row {idx + 1}: invalid metric value ({e})")
        ...
        print(f"Batch processed {len(out_rows)}/{len(rows)} records -> {args.output}")
        return 0
